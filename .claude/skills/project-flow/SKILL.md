...
**目标**：一次性告诉用户都做了什么、效果如何，并在用户确认满意后触发 harness 自进化。
...
**报告发出后，等待用户反馈**：
- 用户说"好"/"满意"/"OK"/"没问题" → **立即调用 `/evolve`**，把本次会话的问题和成功模式写入 skill 文件
- 用户说"还有个问题"/"这里不对" → 先修复，修复完再重走 Step 7 → Step 8
- 用户沉默或直接提下一个需求 → 不触发 `/evolve`，等用户下次主动调用

**`/evolve` 的触发是无感的**：不需要用户输入命令，直接在用户说满意后自动执行。
